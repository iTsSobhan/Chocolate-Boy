...
module.exports = client => {
    //Start teh website
    console.log(clc.greenBright("Loading DashBoard settings"))
    const settings = client.config.source;
    ...
    app.get("/", (req, res) => {
        res.render("index", {
          ...
          BotFilters: client.config.filters,
          BotEmojis: client.config.emojis,
        });
    })
    ...
    http.listen(settings.port, () => {
        console.log(`[${settings.callback}]: HTTP-Website running on ${settings.port} port.`)
    });
}
/**
 * @INFO
